...
6. 选择一个对外展示地址，或者手动输入一个地址；如果双方都有可达 IPv6，可以选择列表里的全局 IPv6，或手动输入 IPv6 地址，邀请码中会保留为 `[IPv6]:PORT`
7. 回到客户端后继续输入房间号和房间密码

这个模式适合单机测试、局域网测试，或者你只是想快速确认功能链路都能跑通。

客户端启动后的连接选择页会轻量检测 IPv6 socket 和默认 IPv6 路由是否可用。服务端启动时会优先尝试 IPv6 dual-stack 监听；系统不支持时会回退到 IPv4，避免破坏已有 IPv4 用法。自建服务器地址列表会从系统路由和本机网卡里发现可展示地址，并过滤 `::1`、`fe80::/10` 链路本地地址、`fc00::/7` 唯一本地地址、文档地址、组播地址和 IPv4 mapped/compatible IPv6 这类不适合发给公网邀请对象的 IPv6。host 流程会进一步显示是否发现公网 IPv6 候选。

如果 A 有可用公网 IPv6，并且本机/路由器防火墙允许外部访问所选 TCP 端口，那么 B/C/D/E 可以通过 A 邀请码中的 `[IPv6]:PORT` 直接加入同一个临时房间聊天。MistV 不会自动绕过阻断入站 TCP 的防火墙，也不会在这个模式里使用 STUN/TURN/ICE、UDP 打洞、UPnP 或 NAT-PMP/PCP；如果对方连接超时，请先确认系统防火墙、路由器 IPv6 防火墙和运营商策略是否允许该端口入站。
...
- 房主在线时可按 `Ctrl+I`、`F2` 或输入 `/invite` 生成一次性邀请码
- 选中附件后按 `Tab` 打开
- 按 `Esc` 退出当前房间
...
1. 房主进入房间后按 `Ctrl+I`、`F2` 或输入 `/invite`
2. 把生成的 `/INVITE:...` 发给对方
3. 新成员启动客户端
